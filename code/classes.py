...
class Mercado:
    ...
    def cadastrar_Cliente(self, nome, numero):
        ...
        cep = requests.get('https://cep.awesomeapi.com.br/json/' + cep)
        cep_json = cep.json()
        while True:
            cpf = validar_Cpf()
            try:
                cursor.execute('INSERT INTO clientes(nome, cpf, cep, estado, bairro, rua, numero)'
                               'VALUES(?,?,?,?,?,?,?)',
                               (nome, cpf, cep, cep_json['state'], cep_json['district'], cep_json['address_name'],
                                numero))
                conexao.commit()
                break
            except:
    ...
    @staticmethod
    def cadastrar_venda(quantidade, valor, cpf, barras):
    ...
    def deletar_clientes(self):
        cursor.execute('DELETE FROM clientes WHERE id>0')
        conexao.commit()

    @staticmethod
    def deletar_vendas(self):
        cursor.execute('DELETE FROM vendas WHERE total>0')
        conexao.commit()
